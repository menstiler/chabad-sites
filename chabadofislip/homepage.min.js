...
function setUpScrolling() {
  window.addEventListener("scroll", function () {
    ...
    if (!e) return;
    const a = e.offsetTop,
      i = document.querySelector("#header .wrapper"),
      t = e.offsetTop + e.offsetHeight;
    window.addEventListener("scroll", () => {
      window.scrollY > t
        ? i.classList.add("scrolled-header")
        : i.classList.remove("scrolled-header");
    }),
      window.pageYOffset > a
        ? i.classList.add("fixed-header")
        : i.classList.remove("fixed-header");
  });
}
function init() {
  setUpScrolling(), setUpSearch();
  const e = document.getElementById("content"),
    a = document.querySelector(".co_content_container.syndicated");
  if (
    (a && document.body.classList.remove("home"),
    "/" === window.location.pathname)
  ) {
    e.insertAdjacentHTML("beforeBegin", textWidgetHtml);
    const a = document.getElementById("HeaderSubscribeContainer");
    if (a) {
      const i = document.querySelector("#HeaderSubscribeContainer form");
      i.classList.remove("subscribe_box"),
        i.classList.add("bottom_subscribe_box"),
        e.after(a);
    }
    e.insertAdjacentHTML("afterend", promosWidgetHtml),
